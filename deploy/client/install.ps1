...
$terminalSource = Join-Path $Source "terminal"
...
$terminalItem = Get-Item -LiteralPath $terminalSource
if (-not $terminalItem.PSIsContainer -or ($terminalItem.Attributes -band [IO.FileAttributes]::ReparsePoint) -or
    -not (Test-Path -LiteralPath (Join-Path $terminalSource "WindowsTerminal.exe") -PathType Leaf) -or
    (Get-ChildItem -LiteralPath $terminalSource -Force -Recurse | Where-Object { $_.Attributes -band [IO.FileAttributes]::ReparsePoint })) {
    throw "Portable terminal input must be a real directory without reparse points"
}
...
foreach ($path in @($root, $bin, $versions, $terminalRoot)) {
...
try {
    ...
    if (-not (Test-Path -LiteralPath $terminalRoot)) {
        $terminalTemporary = Join-Path $root ("terminal.new-" + [Guid]::NewGuid().ToString("N"))
        Copy-Item -LiteralPath $terminalSource -Destination $terminalTemporary -Recurse
        Move-Item -LiteralPath $terminalTemporary -Destination $terminalRoot
        $terminalTemporary = $null
    }
    ...
    if ($null -ne $terminalTemporary -and (Test-Path -LiteralPath $terminalTemporary)) {
        Remove-Item -LiteralPath $terminalTemporary -Recurse -Force
    }
}
