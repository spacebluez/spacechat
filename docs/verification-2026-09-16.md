...
- Windows amd64 客户端版本 0.1.0，构建时注入部署地址，实际地址已省略。
- Linux amd64 服务端已安装到 目标服务器（实际环境信息已省略）。
...
- 监听地址和允许来源网段由部署方配置，实际值已省略。
- 未修改原有防火墙策略；网络访问限制应由部署方按安全策略配置。
...
| go vet ./... | 通过 |
| 目标服务器 临时源码目录，go test -race ./... -count=1 | 通过 |
...
| 本机访问 目标服务器 /healthz | HTTP 200 |
| 两个真实网络客户端连接 目标服务器，发送中文并确认 | 通过 |
...
真实部署测试默认跳过，必须显式设置地址和标记；发送模式会向公共聊天室写入一条测试消息：
    ...
    $env:XCHAT_SMOKE_ADDRESS = 'ws://chat.example.invalid:18080/ws'
...
- 未重启 目标服务器 主机；仅验证开机启动配置已启用，以及服务级重启成功。
