...
| 目标服务器 Linux go test -race ./... -count=1 | 通过 |
...
| 部署后的空/错误密钥测试（目标服务器 及 Windows 发起） | 通过 |
| 部署后的正确密钥双客户端发送确认 | 通过 |
...
- 升级前已进行受限权限备份；实际备份位置由管理员维护，不在公开文档记录。
- 升级前后均保留原有消息；没有提前手动清空生产聊天室。
...
- 未重启 目标服务器 主机；只重启了 XChat 服务完成升级。
