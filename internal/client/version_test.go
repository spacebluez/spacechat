...
func TestClientSendsVersionAndStopsOnRequiredUpgrade(t *testing.T) {
	...
	if peer := network.NewPeer(); peer.info != network.info || peer.token != network.token {
		t.Fatal("room-switch peer lost version metadata or process identity")
	}
	...
	select {
	case join := <-joins:
		if join.ClientVersion != "0.3.2" || join.ClientOS != "windows" || join.ClientArch != "amd64" || len(join.ClientToken) != 64 {
